...
ABM_Runner <- function(pth_str=NULL, household_number=3, random_seed=123){
    ...
    sheds_var_raw <- ABSoutput$sheds_var_raw        # inputs sheds_sheds_variables
    ...
    HUPoutput_temp <- HUP(household_index_str, phf, ent, hpf)
...
# run_mode:
#  1-- non-parallel, directly specify household numbers
#  2-- non-parallel, randomly select a specified number 
#                    of households without replacement
#  3-- parallel, directly specify household numbers
#  4-- parallel, randomly select a specified number 
#                of households without replacement
...
pth_str <- "/home/kdionisio/HEM/ABM/"

# directly specify household numbers
house_list <- 1:3  #house_list <- c(125)
