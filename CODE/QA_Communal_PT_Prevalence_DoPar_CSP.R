...
com_puc_prev<-function(a) { # function to process each PUC. argument a is an index in the list of communal PUCs
  ...
  hd_list<-list.files(path="./OUTPUTS/", pattern="^CPS_out_")
  
  for (hn in hd_list){
    
    hdiary <- read.csv(paste("./OUTPUTS/","/",hn,sep=""))

    # need to remove the refined part
    PUCID.productype <- substr(hdiary$sheds_id, 1, 11)
    print(hn)
    
    print(apuc %in% PUCID.productype)
    
    if (apuc %in% PUCID.productype){
...
write.csv(opdf,"./Communal_Prevalance_CPS.csv", row.names=F)
